...
class Feature(models.Model):
    """Central product catalog entry. A feature is the sellable unit of the
    platform, e.g. Employees, Customers, Billing, Bin Collection."""

    key = models.SlugField(max_length=100, unique=True)
    label = models.CharField(max_length=255)
    description = models.TextField(blank=True)
    entity = models.CharField(max_length=50, blank=True, null=True, unique=True)
...
class Plan(models.Model):
    """A purchasable package of features. Custom deals can still override the
    resulting tenant feature entitlements after a plan is assigned."""

    key = models.SlugField(max_length=100, unique=True)
    name = models.CharField(max_length=255)
    description = models.TextField(blank=True)
    features = models.ManyToManyField(Feature, through='PlanFeature', related_name='plans', blank=True)
    is_active = models.BooleanField(default=True)
    sort_order = models.PositiveIntegerField(default=0)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
...
class TenantModule(models.Model):
    """Tenant feature entitlement. The historical name is kept to avoid
    breaking existing routes/serializers while the product model moves from
    modules to subscription features."""

    tenant = models.ForeignKey(Tenant, on_delete=models.CASCADE, related_name='modules')
    feature = models.ForeignKey(Feature, on_delete=models.CASCADE, related_name='tenant_entitlements', null=True, blank=True)
...
class TenantSubscription(models.Model):
    ...
    plan = models.ForeignKey(Plan, on_delete=models.PROTECT, related_name='tenant_subscriptions', null=True, blank=True)
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default=STATUS_ACTIVE)
    ...
    def __str__(self):
        plan_key = self.plan.key if self.plan else 'custom'
        return f'{self.tenant.slug}:{plan_key}:{self.status}'
